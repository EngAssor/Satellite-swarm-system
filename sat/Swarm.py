...
def Swarm_from_base_to_swarm():
    ...
    client_socket.connect(('192.168.1.4', port))  # connect to the server
...
def Swarm_from_swarm_to_sat():
    ...
    while True :
        try:
            noa=server_socket.bind(('192.168.1.5', port))  # bind host address and port together
        except:
            noa=server_socket.bind(('192.168.1.5',port))
        if noa==None:
            break
